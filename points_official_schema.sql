...
CREATE TABLE livello (
	...
	saga VARCHAR(128), lv_premium integer, price INTEGER DEFAULT 0, elemental_type VARCHAR DEFAULT 'Normal', crit_chance INTEGER DEFAULT 5, crit_multiplier FLOAT DEFAULT 1.5, required_character_id INTEGER, special_attack_name VARCHAR, special_attack_damage INTEGER DEFAULT 0, special_attack_mana_cost INTEGER DEFAULT 0, image_path VARCHAR, telegram_file_id VARCHAR, description VARCHAR, character_group VARCHAR DEFAULT 'General', 
	PRIMARY KEY (id)
);
...
CREATE TABLE mob (
	...
	description VARCHAR, 
	resistance INTEGER, passive_abilities TEXT, active_abilities TEXT, ai_behavior VARCHAR DEFAULT 'aggressive', phase_thresholds TEXT, current_phase INTEGER DEFAULT 1, active_buffs TEXT, 
	PRIMARY KEY (id)
);
CREATE TABLE raid (
	id INTEGER NOT NULL, 
...
CREATE TABLE season (
	...
	description VARCHAR, 
	final_reward_name VARCHAR, theme VARCHAR, 
	PRIMARY KEY (id)
);
